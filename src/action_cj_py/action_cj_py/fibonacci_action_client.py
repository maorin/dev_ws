...
from action_cj_interfaces.action import Fibonacci
            ...
            for i in range(min(max_boxes_to_draw, boxes.shape[0])):
                if scores[i] > min_score_thresh:
                    ...
                    if category_index[class_id]["name"] == "person":
                        ...
                        scores_box.append(scores[i])
                        detection_classes.append(class_id)
            
            
            
            person_box_array = np.array(person_box)
            detection_scores_array = np.array(scores_box)
            detection_classes_array = np.array(detection_classes)

            viz_utils.visualize_boxes_and_labels_on_image_array(
                  image_np_with_detections,
                  person_box_array,
                  detection_classes_array,
                  detection_scores_array,
                  category_index,
                  use_normalized_coordinates=True,
                  max_boxes_to_draw=200,
                  min_score_thresh=.30,
                ...
                if len(det_list) > 0:
                    ...
                    for i, j in zip(row_ind, col_ind):
                        if C[i, j] < self.cost_threshold and j != 0:
                            ...
                            print("id: {}".format(tracks[i, 4]))
                            print("coordinates: %s" % np.around(detections_copy[j-1][0],4))
                            print("tracks_coordinates: %s" % np.around(detections_copy[j-1][1],4))
                            print("cost: {}".format(C[i,j]))
                            """
...
def main(args=None):
    ...
    action_client.objects_detector("name")
